...
def parse_credentials(auth_token: str) -> tuple[str, str]:
    if not auth_token or ":" not in auth_token:
        raise ValueError("auth_token must be in CLIENT_KEY:PASSWORD format")

    client_key, password = auth_token.split(":", 1)
    client_key = client_key.strip()
    password = password.strip()

    if not client_key or not password:
        raise ValueError("CLIENT_KEY or PASSWORD is empty")

    return client_key, password


def hash_sale(client_key: str, password: str, order_id: str, amount_str: str, currency: str) -> str:
    """Digital wallet SALE hash."""
...
def build_sale_payload(
    ...
    payload = {
        ...
        "hash": hash_sale(client_key=client_key, password=password, order_id=order_id, amount_str=amount_str, currency=order_currency),
    }
    for key, val in [
